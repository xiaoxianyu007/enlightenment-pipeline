...
def _ensure_series_files(series_name):
    ...
    if not os.path.exists(src_path):
        log(f"  ✗ 找不到源文件: {bilingual_src}，请先创建")
        sys.exit(1)
    ...
    while True:
        ...
        if not m_en and not m_zh:
            break
        if m_en:
            # EN block: strip the Chinese section inside it
            en_block = m_en.group(0).strip()
            # Remove Chinese section from the English block
    ...
    with open(zh_path, "w", encoding="utf-8") as f:
        f.write("\n\n".join(zh_blocks))
    log(f"  ✓ 已从 {bilingual_src} 拆分为 {cfg['file_en']} / {cfg['file_zh']}")
...
def main():
    ...
    log("╔" + "═" * 58 + "╗")
    log(f"║  {series_name[0]} — Edge-TTS + 双语字幕       ║")
    log("╚" + "═" * 58 + "╝")
    ...
    # ── 阶段4：叠加字幕 ──
    log("叠加字幕 overlay...")
    ep_tag = f"_{args.series}_ep{args.episode}" if args.episode > 0 else f"_{args.series}"
